...
// @VIRTUOS - jboldiga - Needed to modify the mask offsets
static TAutoConsoleVariable<float> CVarUpscaleSoftness(
    TEXT("r.UpscaleSoftness"),
    0.23f,
    TEXT("To scale up with higher quality losing some sharpness\n")
    TEXT(" 0..1 (0.3 is good for ScreenPercentage 90"),
    ECVF_Scalability | ECVF_RenderThreadSafe);
 
// @VIRTUOS - jboldiga - Sharpness scale for bilinear upscaling
static TAutoConsoleVariable<float> CVarSharpBilinearPreScale(
    TEXT("r.SharpBilinearPreScale"),
    4.0f,
    TEXT("Sharpness scale with bilinear upscaling\n")
    TEXT(" 0..100 (4.0f is default)"),
    ECVF_Scalability | ECVF_RenderThreadSafe);
 
// @VIRTUOS - jboldiga - Unsharp mask scale for bilinear upscaling
static TAutoConsoleVariable<float> CVarSharpBilinearUnsharpMaskScale(
    TEXT("r.SharpBilinearUnsharpMaskScale"),
    0.5f,
    TEXT("Unsharp Mask Scale with bilinear upscaling\n")
    TEXT(" 0..5 (0.5f is default)"),
    ECVF_Scalability | ECVF_RenderThreadSafe);
...
public:
    ...
    FShaderParameter UpscaleTexSize;
    FShaderParameter UpscaleSharpBilinearPreScale;
    FShaderParameter UpscaleSharpBilinearUnsharpMaskScale;
        ...
        : FGlobalShader(Initializer)
    {
        ...
        UpscaleTexSize.Bind(Initializer.ParameterMap,TEXT("UpscaleTexSize"));
        UpscaleSharpBilinearPreScale.Bind(Initializer.ParameterMap,TEXT("UpscaleSharpBilinearPreScale"));
        UpscaleSharpBilinearUnsharpMaskScale.Bind(Initializer.ParameterMap,TEXT("UpscaleSharpBilinearUnsharpMaskScale"));
    }
    ...
    void SetPS(const FRenderingCompositePassContext& Context)
    {
        ...
        {
            // @VIRTUOS - jboldiga - Needed for bicubic interpolation and sharp bilinear
            FVector2D UpscaleTexSizeValue(Context.View.UnscaledViewRect.Max.X, Context.View.UnscaledViewRect.Max.Y);
            float UpscaleSharpBilinearPreScaleValue = FMath::Clamp(CVarSharpBilinearPreScale.GetValueOnRenderThread(), 0.0f, 100.0f);
            float UpscaleSharpBilinearUnsharpMaskScaleValue = FMath::Clamp(CVarSharpBilinearUnsharpMaskScale.GetValueOnRenderThread(), 0.0f, 5.0f);
            ...
            SetShaderValue(Context.RHICmdList, ShaderRHI, UpscaleTexSize, UpscaleTexSizeValue);
            SetShaderValue(Context.RHICmdList, ShaderRHI, UpscaleSharpBilinearPreScale, UpscaleSharpBilinearPreScaleValue);
            SetShaderValue(Context.RHICmdList, ShaderRHI, UpscaleSharpBilinearUnsharpMaskScale, UpscaleSharpBilinearUnsharpMaskScaleValue);
        }
    ...
    virtual bool Serialize(FArchive& Ar) override
    {
        ...
        Ar << PostprocessParameter << DeferredParameters << UpscaleSoftness << UpscaleTexSize << UpscaleSharpBilinearPreScale << UpscaleSharpBilinearUnsharpMaskScale;
        return bShaderHasOutdatedParameters;
    }
...
void FRCPassPostProcessUpscale::Process(FRenderingCompositePassContext& Context)
{
    ...
    // @VIRTUOS - jboldiga - if bicubic upscaling is enabled force UpscaleMethod
    UpscaleQuality = 3;
#endif
    ...
    if(bTessellatedQuad)
    {
        switch (UpscaleQuality)
        {
            ...
            case 4: VertexShader = SetShader<4, 1>(Context, CylinderDistortion); break;
            default:
                checkNoEntry();
                break;
        }
    ...
    else
    {
        switch (UpscaleQuality)
        {
            ...
            case 4: VertexShader = SetShader<4, 0>(Context); break;
            default:
                checkNoEntry();
                break;
        }
