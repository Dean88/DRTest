...
#include "PostProcessTonemapUE46.h"
...
#include "PostProcessCombineLUTsUE46.h"
...
#include "PostProcessCustomBlend.h"//@Virtuos[wangsongwei]
 
/** The global center for all post processing activities. */
FPostProcessing GPostProcessing;
 
//@VIRTUOS jboldiga - disable temporal this frame if the resolution has changed
...
static void AddTonemapperUE46(
    FPostprocessContext& Context,
    const FRenderingCompositeOutputRef& BloomOutputCombined,
    const FRenderingCompositeOutputRef& EyeAdaptation)
{
    UE46::FRCPassPostProcessTonemap* PostProcessTonemap = Context.Graph.RegisterPass(new(FMemStack::Get()) UE46::FRCPassPostProcessTonemap(Context.View));
    ...
    {
        FRenderingCompositePass* CombinedLUT = Context.Graph.RegisterPass(new(FMemStack::Get()) UE46::FRCPassPostProcessCombineLUTs(Context.View.GetShaderPlatform()));
...
static void AddGammaCorrectionAfterTonemap(FPostprocessContext& Context)
{
    FRenderingCompositePass* GammeCorrectionTonemap = Context.Graph.RegisterPass(new(FMemStack::Get()) UE46::FRCPassPostProcessGammaCorrectionAfterTonemap(Context.View));
 
    GammeCorrectionTonemap->SetInput(ePId_Input0, Context.FinalOutput);
 
    Context.FinalOutput = FRenderingCompositeOutputRef(GammeCorrectionTonemap);
}
...
//@VIRTUOS jboldiga - Setup the SMAA post process graph
static void AddPostProcessSMAA(FPostprocessContext& Context)
{
    // console variable override
    //static const auto CVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.PostProcessSMAAMethod"));
    //uint32 Quality = FMath::Clamp(CVar->GetValueOnRenderThread(), 0, 2);
    uint32 Quality = 0;
 
    // edge detection
    FRenderingCompositePass* EdgeDetection = Context.Graph.RegisterPass(new(FMemStack::Get()) FRCPassPostProcessSMAAEdgeDetection(Quality));
    EdgeDetection->SetInput(ePId_Input0, FRenderingCompositeOutputRef(Context.FinalOutput));
 
    // blend weights
    FRenderingCompositePass* BlendWeights = Context.Graph.RegisterPass(new(FMemStack::Get()) FRCPassPostProcessSMAABlendWeights());
    BlendWeights->SetInput(ePId_Input0, FRenderingCompositeOutputRef(FRenderingCompositeOutputRef(EdgeDetection)));
 
    // neighborhood blending
    FRenderingCompositePass* NeighborhoodBlending = Context.Graph.RegisterPass(new(FMemStack::Get()) FRCPassPostProcessSMAANeighborhoodBlending());
    NeighborhoodBlending->SetInput(ePId_Input0, FRenderingCompositeOutputRef(Context.FinalOutput));
    NeighborhoodBlending->SetInput(ePId_Input1, FRenderingCompositeOutputRef(FRenderingCompositeOutputRef(BlendWeights)));
 
    // final composite output
#if DEBUG_SMAA
    static bool edges =  true;
    static bool blends = false;
 
    if(edges)
        Context.FinalOutput = FRenderingCompositeOutputRef(EdgeDetection);
    if(blends)
        Context.FinalOutput = FRenderingCompositeOutputRef(BlendWeights);
    if(edges && blends)
        Context.FinalOutput = FRenderingCompositeOutputRef(NeighborhoodBlending);
#else
    Context.FinalOutput = FRenderingCompositeOutputRef(NeighborhoodBlending);
#endif
}
...
static void AddTemporalAA( FPostprocessContext& Context, FRenderingCompositeOutputRef& VelocityInput )
{
    ...
    FRenderingCompositePass* HistoryInput;
#if ENABLE_BICUBIC_UPSCALING
    if( ViewState && ViewState->TemporalAAHistoryRT && !Context.View.bCameraCut && !GResolutionChanged )
#else
...
#define AFFECT_DETECTIVE_MODE_RT_PRIORITY (2000)
static void AddDMRTBlendProcess(FPostprocessContext& Context,FRenderingCompositeOutputRef CustomSeparateTranslucency, FRenderingCompositeOutputRef CustomSpecialTranslucency)
{
    const FSceneView& View = Context.View;
    //Note: here we disable all the blend action in normal mode, that's to say, the object in new-sub-pass will just appear in special mode, this maybe cause some component disappear in normal mode!!!
    bool bAllowFullPostProcessing = View.Family->EngineShowFlags.PostProcessing
        && View.GetFeatureLevel() >= ERHIFeatureLevel::SM4
        && !View.Family->EngineShowFlags.VisualizeDistanceFieldAO
        && !View.Family->EngineShowFlags.VisualizeDistanceFieldGI
        && !View.Family->EngineShowFlags.VisualizeMeshDistanceFields;
    if ((View.bIsXray || View.bIsThermal || View.bIsScanMode) && bAllowFullPostProcessing /*&& (GSceneRenderTargets.IsCustomSeparateTranslucencyActive(Context.View) || GSceneRenderTargets.IsCustomSpecialTranslucencyActive(Context.View))*/)
    {
        FRenderingCompositePass* NodeCustomTranslucency = Context.Graph.RegisterPass(new(FMemStack::Get()) FRCPassPostCustomBlend(true));
        NodeCustomTranslucency->SetInput(ePId_Input0, Context.FinalOutput);
        NodeCustomTranslucency->SetInput(ePId_Input1, Context.FinalOutput);
        NodeCustomTranslucency->SetInput(ePId_Input2, CustomSeparateTranslucency);
        NodeCustomTranslucency->SetInput(ePId_Input3, CustomSpecialTranslucency);
 
        Context.FinalOutput = FRenderingCompositeOutputRef(NodeCustomTranslucency);
    }
}
//@Virtuos[wangsongwei]
static void AddPostProcessMaterial(FPostprocessContext& Context, EBlendableLocation InLocation, FRenderingCompositeOutputRef SeparateTranslucency, FRenderingCompositeOutputRef CustomSeparateTranslucency = NULL, FRenderingCompositeOutputRef CustomSpecialTranslucency = NULL)//@Virtuos[wangsongwei]
{
    if(!Context.View.Family->EngineShowFlags.PostProcessing || !Context.View.Family->EngineShowFlags.PostProcessMaterial)
    {
        return;
    ...
    bool Has_Add_DM_Blend_Process = false;
    for(uint32 i = 0; i < PPNodeCount; ++i)
    {
        //@Virtuos[wangsongwei]
        if (!Has_Add_DM_Blend_Process && InLocation == BL_AfterTonemapping && PPNodes[i].GetPriority() > AFFECT_DETECTIVE_MODE_RT_PRIORITY)
        {
            AddDMRTBlendProcess(Context,CustomSeparateTranslucency,CustomSpecialTranslucency);
            Has_Add_DM_Blend_Process = true;
        }
        //@Virtuos[wangsongwei]
    ...
    //@Virtuos[wangsongwei]begin:
    if (!Has_Add_DM_Blend_Process && InLocation == BL_AfterTonemapping)
    {
        AddDMRTBlendProcess(Context,CustomSeparateTranslucency,CustomSpecialTranslucency);
        Has_Add_DM_Blend_Process = true;
    }
...
void FPostProcessing::Process(FRHICommandListImmediate& RHICmdList, FViewInfo& View, TRefCountPtr<IPooledRenderTarget>& VelocityRT)
{
    ...
    // so that the passes can register themselves to the graph
    {
        ...
        //@Virtuos[wangsongwei]begin:
        FRenderingCompositeOutputRef CustomSeparateTranslucency;
        FRenderingCompositeOutputRef CustomSpecialTranslucency;
        //@Virtuos[wangsongwei]end
        // optional
        ...
        //@Virtuos[wangsongwei]begin:
        {
            if(ViewState && ViewState->CustomSeparateTranslucencyRT)
            {
                FRenderingCompositePass* NodeCustomSeparateTranslucency = Context.Graph.RegisterPass(new(FMemStack::Get()) FRCPassPostProcessInput(ViewState->CustomSeparateTranslucencyRT));
 
                CustomSeparateTranslucency = FRenderingCompositeOutputRef(NodeCustomSeparateTranslucency);
                // the node keeps another reference so the RT will not be release too early
                ViewState->FreeCustomSeparateTranslucency();
 
                check(!ViewState->CustomSeparateTranslucencyRT);
            }
        }
        {
            if(ViewState && ViewState->CustomSpecialTranslucencyRT)
            {
                FRenderingCompositePass* NodeCustomSpecialTranslucency = Context.Graph.RegisterPass(new(FMemStack::Get()) FRCPassPostProcessInput(ViewState->CustomSpecialTranslucencyRT));
 
                CustomSpecialTranslucency = FRenderingCompositeOutputRef(NodeCustomSpecialTranslucency);
                // the node keeps another reference so the RT will not be release too early
                ViewState->FreeCustomSpecialTranslucency();
        ...
        if (AllowFullPostProcessing(View, FeatureLevel))
        {
            ...
            bDepthOfField = bDepthOfField && !View.bIsXray;     // @VIRTUOS [GuiQing] porting from AA - BMBUG-14406 - Dof should not be enabled when batman takedown with XRay on.
#endif
            ...
            if(bAllowTonemapper && FeatureLevel >= ERHIFeatureLevel::SM4)
            {
                ...
                {
                    AddTonemapperUE46(Context, BloomOutputCombined, EyeAdaptation);
                }
            }
            ...
            else if(AntiAliasingMethod == AAM_SMAA)
            {
                AddPostProcessSMAA(Context);
            }
        ...
        AddPostProcessMaterial(Context, BL_AfterTonemapping, SeparateTranslucency, CustomSeparateTranslucency, CustomSpecialTranslucency);//@Virtuos[wangsongwei]
...
#if VIRTUOS_PAIR_UE3UE4 && GAMMA_CORRECTION_AFTER_TONEMAP
        AddGammaCorrectionAfterTonemap(Context);    // @VIRTUOS [GuiQing] do gamma correction here to ensure all the PP effects get covered.
#endif
