...
FSceneView::FSceneView(const FSceneViewInitOptions& InitOptions)
    ...
    , bIsXray(false)        // @VIRTUOS [RS]
    , bIsThermal(false)     // @VIRTUOS [RS]
    , bIsScanMode(false)    // @VIRTUOS [RS]
    , bIsReflection(false)  // @VIRTUOS [RS]
    , bIsLocked(false)
    ...
    , bEnableCapeDepthBiasHack(false)
{
    ...
    //@VIRTUOS [BG] UE3 Rock modifications for measuring distance, used in rope system
    CameraPlane = FPlane(InvViewMatrix.GetOrigin(), InvViewMatrix.GetScaledAxis(EAxis::Z));
...
void FSceneView::OverridePostProcessSettings(const FPostProcessSettings& Src, float Weight)
{
    ...
    if(Dest.DepthOfFieldMethod == Src.DepthOfFieldMethod)
    {
        LERP_PP(DepthOfFieldNearMaxBlurAmount);
        LERP_PP(DepthOfFieldFarMaxBlurAmount);
    }
    if(Src.DepthOfFieldMethod == DOFM_Gaussian)
    {
        Dest.DepthOfFieldNearMaxBlurAmount = Src.DepthOfFieldNearMaxBlurAmount * Weight;
        Dest.DepthOfFieldFarMaxBlurAmount = Src.DepthOfFieldFarMaxBlurAmount * Weight;
    }
    ...
    {
        //@VIRTUOS jboldiga - Do not use with cinematics branch!
#if WITH_EDITOR
        Dest.AntiAliasingMethod = Src.AntiAliasingMethod;
#else
        Dest.AntiAliasingMethod = AAM_SMAA;
#endif
    }
...
void FSceneView::EndFinalPostprocessSettings(const FSceneViewInitOptions& ViewInitOptions)
{
    ...
    // Anti-Aliasing
    {
        ...
        // @Virtuos[LiuHuan]: We should never use FXAA.
...
//          }
//      }
        // @VIRTUOS jboldiga - We should use SMAA if TemporalAA is not available
        if( FinalPostProcessSettings.AntiAliasingMethod == AAM_TemporalAA)
        {
            if( !Family->EngineShowFlags.TemporalAA || !Family->bRealtimeUpdate || Quality < 3 )
            {
                FinalPostProcessSettings.AntiAliasingMethod = AAM_SMAA;
            }
        }
    }
