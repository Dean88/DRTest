...
#include "GFxUIRendererSceneRenderTargetProxy.h"
 
//@VIRTUOS jboldiga - Needed for dynamic resolution
#if ENABLE_BICUBIC_UPSCALING
bool GAllocatedRenderTargets = false;
bool GResolutionChanged = false;
extern float GBicubicUpscalingPercentageX;
#endif
 
// @Virtuos [Ding Yifei]: Use custom fixed resolution to avoid abrupt reflection refresh on glass and mirror in chapter one
...
void FSceneRenderTargets::Allocate(const FSceneViewFamily& ViewFamily)
{
    ...
    GResolutionChanged = FPlatformMisc::DynamicResolution(&GBicubicUpscalingPercentageX);
    ...
    if( ((BufferSize.X != DesiredBufferSize.X) ||
        ...
        (CurrentMinShadowResolution != MinShadowResolution)) && !GAllocatedRenderTargets)
#else
    if( (BufferSize.X != DesiredBufferSize.X) ||
        (BufferSize.Y != DesiredBufferSize.Y) ||
        (CurrentGBufferFormat != GBufferFormat) ||
...
int32 FSceneRenderTargets::GetGBufferRenderTargets(ERenderTargetLoadAction ColorLoadAction, FRHIRenderTargetView OutRenderTargets[MaxSimultaneousRenderTargets], int32& OutVelocityRTIndex)
{
    ...
    int32 MRTCount = 4; // Derived from previously used RenderTargets[]; would have been nice to keep a pointer while filling it but it's more confusing to use
...
// @Virtuos [wangsongwei] begin: for detective mode new stencil written pass
bool FSceneRenderTargets::BeginRenderingXRayStencil(FRHICommandListImmediate& RHICmdList)
{
    //since the xray stencil pass' result(one stencil buffer) will be used in translucent pass, they should use the same one render target 
    // Use one dummy scene color buffer.
    //SetRenderTarget(RHICmdList, FTextureRHIRef(), GetSceneDepthSurface()/*, ESimpleRenderTargetMode::EClearColorToBlack, FExclusiveDepthStencil::DepthRead_StencilWrite*/);
    ...
    // set stencil reference and mask, to xray use 0x40 here
    RHICmdList.SetDepthStencilState(TStaticDepthStencilState<false, CF_Equal/*CF_GreaterEqual*/, true, CF_Always, SO_Replace, SO_Keep, SO_Replace, false, CF_Always, SO_Replace, SO_Keep, SO_Replace, 0xFF, 0X40>::GetRHI(), 0x40);
 
    return true;
}
void FSceneRenderTargets::FinishRenderingXRayStencil(FRHICommandListImmediate& RHICmdList, const FResolveRect& ResolveRect)
{
    ResolveSceneDepthTexture(RHICmdList);
...
//@Vituros[wangsongwei]begin:
void FSceneRenderTargets::BeginRenderingCustomPrePass(FRHICommandList& RHICmdList)
{
    SCOPED_DRAW_EVENT(RHICmdList, BeginRenderingCustomPrePass);
         
    // Set the scene depth surface and a DUMMY buffer as color buffer
    // (as long as it's the same dimension as the depth buffer),   
    SetRenderTarget(RHICmdList, FTextureRHIRef(), GetSceneDepthSurface());
}
void FSceneRenderTargets::FinishRenderingCustomPrePass(FRHICommandListImmediate& RHICmdList)
{
    SCOPED_DRAW_EVENT(RHICmdList, FinishRenderingCustomPrePass);
     
    ResolveSceneDepthTexture(RHICmdList);
    //ResolveSceneDepthToAuxiliaryTexture();
...
bool FSceneRenderTargets::BeginRenderingCustomSeparateTranslucency(FRHICommandList& RHICmdList, const FViewInfo& View, bool bFirstTimeThisFrame)
{
    if(IsCustomSeparateTranslucencyActive(View))
    {
        SCOPED_DRAW_EVENT(RHICmdList, BeginCustomSeparateTranslucency);
        ...
        if(bFirstTimeThisFrame)
        {
            check(!ViewState->CustomSeparateTranslucencyRT);
        }
 
        TRefCountPtr<IPooledRenderTarget>& CustomSeparateTranslucency = ViewState->GetCustomSeparateTranslucency(View);
 
        // clear the render target the first time, re-use afterwards
        SetRenderTarget(RHICmdList, CustomSeparateTranslucency->GetRenderTargetItem().TargetableTexture, GetSceneDepthSurface(),
            bFirstTimeThisFrame ? ESimpleRenderTargetMode::EClearColorToBlackWithFullAlpha : ESimpleRenderTargetMode::EExistingColorAndDepth, FExclusiveDepthStencil::DepthRead_StencilWrite);
...
void FSceneRenderTargets::FinishRenderingCustomSeparateTranslucency(FRHICommandList& RHICmdList, const FViewInfo& View)
{
    if(IsCustomSeparateTranslucencyActive(View))
    {
        SCOPED_DRAW_EVENT(RHICmdList, FinishCustomSeparateTranslucency);
 
        FSceneViewState* ViewState = (FSceneViewState*)View.State;
        TRefCountPtr<IPooledRenderTarget>& CustomSeparateTranslucency = ViewState->GetCustomSeparateTranslucency(View);
 
        RHICmdList.CopyToResolveTarget(CustomSeparateTranslucency->GetRenderTargetItem().TargetableTexture, CustomSeparateTranslucency->GetRenderTargetItem().ShaderResourceTexture, true, FResolveParams());
    }
}
bool FSceneRenderTargets::BeginRenderingCustomSpecialTranslucency(FRHICommandList& RHICmdList, const FViewInfo& View, bool bFirstTimeThisFrame, bool bFirstPassRenderCustomSpecialTranslucency)
{
    if(IsCustomSpecialTranslucencyActive(View))
    {
        SCOPED_DRAW_EVENT(RHICmdList, BeginCustomSpecialTranslucency);
 
        FSceneViewState* ViewState = (FSceneViewState*)View.State;
 
        // the RT should only be available for a short period during rendering
        if(bFirstTimeThisFrame && bFirstPassRenderCustomSpecialTranslucency)
        {
            check(!ViewState->CustomSpecialTranslucencyRT);
        }
 
        TRefCountPtr<IPooledRenderTarget>& CustomSpecialTranslucency = ViewState->GetCustomSpecialTranslucency(View);
 
        // clear the render target the first time, re-use afterwards
        SetRenderTarget(RHICmdList, CustomSpecialTranslucency->GetRenderTargetItem().TargetableTexture, GetSceneDepthSurface(),
            (bFirstTimeThisFrame&&bFirstPassRenderCustomSpecialTranslucency) ? ESimpleRenderTargetMode::EClearColorToBlackWithFullAlpha : ESimpleRenderTargetMode::EExistingColorAndDepth, FExclusiveDepthStencil::DepthRead_StencilWrite);
...
void FSceneRenderTargets::FinishRenderingCustomSpecialTranslucency(FRHICommandList& RHICmdList, const FViewInfo& View)
{
    if(IsCustomSpecialTranslucencyActive(View))
    {
        SCOPED_DRAW_EVENT(RHICmdList, FinishCustomSpecialTranslucency);
 
        FSceneViewState* ViewState = (FSceneViewState*)View.State;
        TRefCountPtr<IPooledRenderTarget>& CustomSpecialTranslucency = ViewState->GetCustomSpecialTranslucency(View);
 
        RHICmdList.CopyToResolveTarget(CustomSpecialTranslucency->GetRenderTargetItem().TargetableTexture, CustomSpecialTranslucency->GetRenderTargetItem().ShaderResourceTexture, true, FResolveParams());
    }
}
//@Virtuos[wangsongwei]end
...
void FSceneRenderTargets::ResolveSceneDepthToAuxiliaryTexture(FRHICommandList& RHICmdList)
{
    ...
    if(!GSupportsDepthFetchDuringDepthTest || GUseAuxiliarySceneDepthInRenderLights) //@Virtuos [Zhangheng_a]: Use Auxiliary SceneDepth In RenderLights to reduce depth decompress time.
    {
...
//@Virtuos[wangsongwei]begin:
bool FSceneRenderTargets::IsCustomSeparateTranslucencyActive(const FViewInfo& View) const
{
    //static const auto CVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.SeparateTranslucency"));
    //int32 Value = CVar->GetValueOnRenderThread();
    ...
    return /*(Value != 0) && */CurrentFeatureLevel >= ERHIFeatureLevel::SM4
        && View.Family->EngineShowFlags.PostProcessing
        && View.Family->EngineShowFlags.CustomSeparateTranslucency
        && View.State != NULL;  // We require a ViewState in order for separate translucency to work (it keeps track of our CustomSeparateTranslucencyRT)
}
bool FSceneRenderTargets::IsCustomSpecialTranslucencyActive(const FViewInfo& View) const
{
    //static const auto CVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.SeparateTranslucency"));
    //int32 Value = CVar->GetValueOnRenderThread();
    ...
    return /*(Value != 0) && */CurrentFeatureLevel >= ERHIFeatureLevel::SM4
        && View.Family->EngineShowFlags.PostProcessing
        && View.Family->EngineShowFlags.CustomSpecialTranslucency
        && View.State != NULL;  // We require a ViewState in order for separate translucency to work (it keeps track of our CustomSpecialTranslucencyRT)
}
//@Virtuos[wangsongwei]end
...
static TCHAR* const GetVolumeName(uint32 Id, bool bDirectional) //@Virtuos [Zhangheng_a]: Lightchannel implement for Translucent render.
{
    // [+] Statically construct all possible names as constants
#define tmp_GetVolumeName_get_name_amb(light_channel_mask, real_id) \
    (TEXT("LightChannelMask") TEXT(#light_channel_mask) \
     TEXT("+TranslucentVolume") TEXT(#real_id))
#define tmp_GetVolumeName_get_name_dir(light_channel_mask, real_id) \
    (TEXT("LightChannelMask") TEXT(#light_channel_mask) \
     TEXT("+TranslucentVolumeDir") TEXT(#real_id))
#define tmp_GetVolumeName_get_names_for_light_channel(light_channel_mask) \
    { \
        { \
            tmp_GetVolumeName_get_name_amb(light_channel_mask, 0), \
            tmp_GetVolumeName_get_name_dir(light_channel_mask, 0), \
        }, \
        { \
            tmp_GetVolumeName_get_name_amb(light_channel_mask, 1), \
            tmp_GetVolumeName_get_name_dir(light_channel_mask, 1), \
        }, \
        { \
            tmp_GetVolumeName_get_name_amb(light_channel_mask, 2), \
            tmp_GetVolumeName_get_name_dir(light_channel_mask, 2), \
        }, \
    }
    static const TCHAR * const
        VolumeNameTable[LIGHTCHANNELMASK_USED_MAX][TVC_MAX + 1][2] = {
            tmp_GetVolumeName_get_names_for_light_channel(0),
            tmp_GetVolumeName_get_names_for_light_channel(1),
            tmp_GetVolumeName_get_names_for_light_channel(2),
            tmp_GetVolumeName_get_names_for_light_channel(3),
            tmp_GetVolumeName_get_names_for_light_channel(4),
            tmp_GetVolumeName_get_names_for_light_channel(5),
            tmp_GetVolumeName_get_names_for_light_channel(6),
            tmp_GetVolumeName_get_names_for_light_channel(7),
        };
    // FIXME: The checks could be made static_assert() if the compiler supports
    //        C++11 "constexpr", and Visual Studio 2012 unfortunately is not
    //        one of them
    checkf(VolumeNameTable[0][0][0],
            TEXT("VolumeNameTable is not completely filled"));
    checkf(VolumeNameTable
        [ARRAY_COUNT(VolumeNameTable) - 1]
        [ARRAY_COUNT(VolumeNameTable[0]) - 1]
        [ARRAY_COUNT(VolumeNameTable[0][0]) - 1],
        TEXT("VolumeNameTable is not completely filled"));
 
#undef tmp_GetVolumeName_get_names_for_light_channel
#undef tmp_GetVolumeName_get_name_amb
#undef tmp_GetVolumeName_get_name_dir
    // [-] Statically construct all possible names as constants
 
    uint32 TranslucencyVolumeIndex = Id / (TVC_MAX + 1); //@Virtuos [Zhangheng_a]: Reduce TranslucentLightingVolumes
    uint32 RealId = Id % (TVC_MAX + 1);
 
    check(TranslucencyVolumeIndex >= 0 && TranslucencyVolumeIndex < LIGHTCHANNELMASK_USED_MAX);
 
    const TCHAR *name = VolumeNameTable[TranslucencyVolumeIndex][RealId][bDirectional];
...
void FSceneRenderTargets::AllocateCommonDepthTargets()
{
    ...
    if (!AuxiliarySceneDepthZ && (!GSupportsDepthFetchDuringDepthTest || GUseAuxiliarySceneDepthInRenderLights)) //@Virtuos [Zhangheng_a]: Use Auxiliary SceneDepth In RenderLights to reduce depth decompress time.
    {
        FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(BufferSize, PF_DepthStencil, TexCreate_None, TexCreate_DepthStencilTargetable, false));
...
{
//@VIRTUOS [BG] BEGIN: Set the shadow buffer resolution with a static value, to optimize the shadow buffer
#if DINGO
    int32 MaxShadowRes = MaxShadowBufferResolution;
#else
...
int32 FSceneRenderTargets::GetTranslucencyVolumeIndex(uint32 LightChannelMask) const //@Virtuos [Zhangheng_a]: Reduce TranslucentLightingVolumes
{
    check(LightChannelMask >= 0 && LightChannelMask <= LIGHTCHANNELMASK_MAX);
 
    for(uint32 TranslucencyVolumeIndex = 0; TranslucencyVolumeIndex < LIGHTCHANNELMASK_USED_MAX; TranslucencyVolumeIndex++)
    {
        if(LightChannelMask == LightChannelMaskUsed[TranslucencyVolumeIndex])
        {
            return TranslucencyVolumeIndex;
        }
    }
...
int32 FSceneRenderTargets::SetTranslucencyVolumeIndex(uint32 LightChannelMask) //@Virtuos [Zhangheng_a]: Reduce TranslucentLightingVolumes
{
    check(LightChannelMask >= 0 && LightChannelMask <= LIGHTCHANNELMASK_MAX);
    ...
    for(uint32 TranslucencyVolumeIndex = 0; TranslucencyVolumeIndex < LIGHTCHANNELMASK_USED_MAX; TranslucencyVolumeIndex++)
    {
        ...
        if(LightChannelMaskUsed[TranslucencyVolumeIndex] == -1)
        {
            LightChannelMaskUsed[TranslucencyVolumeIndex] = LightChannelMask;
            return TranslucencyVolumeIndex;
        }
    }
...
#if  (VIRTUOS_PAIR_UE3UE4 && USE_UE4GFXUIRENDERER_IN_PAIR)
/*-----------------------------------------------------------------------------
...
*/
/*
void FSceneRenderTargetProxy::SetSizes(uint32 InSizeX, uint32 InSizeY)
{
    SizeX = InSizeX;
    SizeY = InSizeY;
...
*/
const FTexture2DRHIRef& FSceneRenderTargetProxy::GetRenderTargetTexture() const
{
    return (const FTexture2DRHIRef&)GSceneRenderTargets.GetSceneColorSurface();
}
 
/**
* @return width and height of the scene render target this proxy will render to
*/
FIntPoint FSceneRenderTargetProxy::GetSizeXY() const
{
...
*/
uint32 FSceneRenderTargetProxy::GetSizeX() const
{
    return SizeX;
}
...
*/
uint32 FSceneRenderTargetProxy::GetSizeY() const
{
    return SizeY;
}
...
/*-----------------------------------------------------------------------------
FSceneDepthTargetProxy
-----------------------------------------------------------------------------*/
 
TRefCountPtr<IPooledRenderTarget> GFxSceneDepthZ;
 
/**
* Set SizeX and SizeY of proxy and re-allocate depth target as needed
*
* @param InSizeX - depth target width requested
* @param InSizeY - depth target height requested
*/
void FSceneDepthTargetProxy::SetSizes(uint32 InSizeX, uint32 InSizeY)
{
    ...
    if (!GFxSceneDepthZ)
    {
        FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(FIntPoint(InSizeX, InSizeY), PF_DepthStencil, TexCreate_None, TexCreate_DepthStencilTargetable, false));
        GRenderTargetPool.FindFreeElement(Desc, GFxSceneDepthZ, TEXT("GFxSceneDepthZ"));
        UE_LOG(LogRenderer, Log, TEXT("GFxSceneDepthZ Created Size = %d"), GFxSceneDepthZ->ComputeMemorySize());
    }
    else if(GFxSceneDepthZ && (GFxSceneDepthZ->GetDesc().Extent.X < (int32)InSizeX || GFxSceneDepthZ->GetDesc().Extent.Y < (int32)InSizeY))
    {
        GFxSceneDepthZ.SafeRelease();
        FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(FIntPoint(InSizeX, InSizeY), PF_DepthStencil, TexCreate_None, TexCreate_DepthStencilTargetable, false));
        GRenderTargetPool.FindFreeElement(Desc, GFxSceneDepthZ, TEXT("GFxSceneDepthZ"));
        UE_LOG(LogRenderer, Log, TEXT("GFxSceneDepthZ Resized Size = %d"), GFxSceneDepthZ->ComputeMemorySize());
    }
}
...
* @return RHI surface for setting the render target
*/
const FTextureRHIRef& FSceneDepthTargetProxy::GetDepthTargetSurface() const
{
    return (const FTextureRHIRef&)GFxSceneDepthZ->GetRenderTargetItem().TargetableTexture;
}
 
#endif //(VIRTUOS_PAIR_UE3UE4 && USE_UE4GFXUIRENDERER_IN_PAIR)