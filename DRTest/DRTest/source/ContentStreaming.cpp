...
//@ROCK - Dustin: Tweak streaming texel factors for LOD textures
float GNearLODStreamingFactor = 1.0f;
float GFarLODStreamingFactor = 1.0f;
...
#define MAX_STREAMINGDISTANCE   13000.0f
#define MAX_MIPDELTA            5.0f
#define MAX_LASTRENDERTIME      90.0f
 
//@ROCK - Dustin: Limit the number of texture load requests we handle at a time, to ensure the highest priority requests are loaded first
#define MAX_TEXTURE_LOAD_REQUESTS 16
 
//@ROCK - Dustin: Added support for different package priorities
#define HIGH_TEXTURE_PRIORITY 2.0f
 
//@ROCK - Dustin: Used for more accurate texture priority calculations
#define MAX_TEXEL_SIZE 64.0f
 
 
#ifdef VIRTUOS_PAIR_UE3UE4 
//[DP] We have better streaming texture performance with this variable at false.
//FStreamingHandlerTextureLastRender::GetWantedMips return a wanted mips at 0 with this parameter
//@VIRTUOS [lizimian]: Change back to TRUE, otherwise it blocks particle referenced textures' streaming.
...
// Virtuos SHL: ignore bicubic upscaling percentage in screen size
...
int32 FStreamingManagerTexture::NumTexturesLoading = 0; //@ROCK
 
int32 ExtraMemoryMargin = 0; //@ROCK - Dustin: Allow extra memory margain; e.g. when we switch to 3D mode
 
#if PLATFORM_XBOXONE
extern void gCallProcessEvents();
#endif
...
struct FStreamingTexture
{
    FStreamingTexture( UTexture2D* InTexture )
    {
        ...
        LODSubGroup = (TextureSubGroup) InTexture->LODSubGroup;
    ...
    {
        // @VIRTUOS [BG] Get resident mip count based on Mip data
        return Texture2D && Texture2D->bIsStreamable &&
            Texture2D->NeverStream == false && Texture2D->GetNumMips() > FMath::Max(UTexture2D::GetMinTextureResidentMipCount(Texture2D->GetPlatformMips(),Texture2D->LODGroup, Texture2D->LODSubGroup), UDeviceProfileManager::Get().GetActiveProfile()->GetTextureLODSettings()->GetMinLODMipCount(Texture2D->LODGroup, Texture2D->LODSubGroup)); //@ROCK - Dustin: Added support for per LOD group min texture resident mip count
        //return Texture2D && Texture2D->bIsStreamable &&
            //Texture2D->NeverStream == false && Texture2D->GetNumMips() > FMath::Max(UTexture2D::GetMinTextureResidentMipCount(), UDeviceProfileManager::Get().GetActiveProfile()->GetTextureLODSettings()->GetMinLODMipCount(Texture2D->LODGroup, Texture2D->LODSubGroup)); //@ROCK - Dustin: Added support for per LOD group min texture resident mip count
    }
    ...
    {
        check(InMipCount > 0 && InMipCount <= MAX_TEXTURE_MIP_COUNT); //@ROCK - BW: Changed >= to > to avoid it reading data outside of the array
    ...
    float CalcPriority( )
    {
        if (LODGroup != TEXTUREGROUP_Terrain_Heightmap)
        {
            ...
            //@ROCK - Dustin: Tuned the maths behind this whole function, see me before merging!
            float Priority=0.0f;
            float TimeFactor = 1.0f - FMath::Sqrt( FMath::Min(LastRenderTime, MAX_LASTRENDERTIME) / MAX_LASTRENDERTIME );
 
            if( bUsesLastRenderHeuristics )
            {
                Priority = TimeFactor;
            }
            else
            {
                //For any world/dynamic textures, add in a distance factor to account for textures close to the camera but not currently rendered
                float ScreenSizeFactor =FMath::Min(FMath::Sqrt(ScreenSizeInTexels) / MAX_TEXEL_SIZE, 1.0f);
                float DistanceFactor = 1.0f - FMath::Sqrt( FMath::Min(MinDistance, MAX_STREAMINGDISTANCE) / MAX_STREAMINGDISTANCE );
                Priority = (0.5f + TimeFactor) * ScreenSizeFactor + DistanceFactor;
            }
 
            //Characters should have a higher priority than world textures where possible
            if( LODGroup == TEXTUREGROUP_Character || LODGroup == TEXTUREGROUP_CharacterNormalMap || LODGroup == TEXTUREGROUP_CharacterSpecular)
                Priority *= 2.0f;
 
            // @Virtuos Andy: A hardcode for fixing BMBUG-13764: [WB Bug 3948][PS4] - AC - SS: Watcher in the Wings (Azrael) - OW_A5 - Streaming visible on the symbol appearing on the Church wall.
            // Raise the priority of a texture to make it loaded earlier. If other fix (like texel factor) can fix this issue we can revert this.
            static FName name = TEXT("Game_Azrael_Glyphs_D");
            if( Texture && Texture->GetFName() == name )
            {
                Priority *= 2.0f;
            }
            Priority = BoostFactor * Priority + bForceFullyLoad * 100.0f;
            return Priority;
    ...
    bool IsLODTextureHidden() const
    {
        return LastRenderTime > 5.0f && ((LODGroup == TEXTUREGROUP_ProcBuilding_Face && MinDistance < 3000.0f) || (LODGroup == TEXTUREGROUP_ProcBuilding_LightMap && MinDistance < 15000.0f));
    }
    ...
    //@Rock - JN - mod to remove expensive cost of map search for each texture in GetWantedMips2
    struct TextureInstanceInfoData
    {
        FStreamingManagerTexture::FLevelData* LevelData;
        uint32 TextureInstancesIndex;
    };
 
    TArray<TextureInstanceInfoData> ThreadTextureInstancesData;  
 
    inline int32 GetNumTextureInstances() const { return ThreadTextureInstancesData.Num(); }
    inline FStreamingManagerTexture::FLevelData* GetLevelData(int32 i) const { return ThreadTextureInstancesData[i].LevelData; }
    inline int32 GetTextureInstancesIndex(int32 i) const { return ThreadTextureInstancesData[i].TextureInstancesIndex; }
    inline void AddTextureInstanceInfo( FStreamingManagerTexture::FLevelData* LevelData, int32 TextureInstancesIndex)
    {
        TextureInstanceInfoData Data;
        Data.LevelData = LevelData;
        Data.TextureInstancesIndex = TextureInstancesIndex;
 
        ThreadTextureInstancesData.Add(Data);
    }
    ...
    //@ROCK - Dustin: Added support for per LOD group min texture resident mip count
    int32               MinTextureResidentMipCount;
 
    /** Cached texture group. */
    TextureGroup    LODGroup;
    TextureSubGroup LODSubGroup;
    /** Cached LOD bias. */
    ...
    /** @VIRTUOS [TV] Whether this texture is currently in high priority */
    uint32          bMAXPriority : 1;
    /** @VIRTUOS [DP] Save previous requested priority*/
    uint32          bPreviousPriority : 1;
};
...
struct FStreamingContext
{
    ...
    void Reset( bool bProcessEverything, UTexture2D* IndividualStreamingTexture, bool bInCollectTextureStats )
    {
        ...
        if( bRHISupportsMemoryStats )
        {
            ...
            SET_MEMORY_STAT(STAT_TexturePoolAllocatedSize_Custom, AllocatedMemorySize);
            SET_MEMORY_STAT(STAT_TexturePoolSize_Custom, Stats.TexturePoolSize);
        }
        else
        {
            ...
            SET_MEMORY_STAT(STAT_TexturePoolAllocatedSize_Custom,0);
            SET_MEMORY_STAT(STAT_TexturePoolSize_Custom, 0);
        }
    }
...
private:
    ...
    void DoWork()
    {
        ...
        TM_SCOPE("FAsyncTextureStreaming::DoWork()");
#if VIRTUOS_PAIR_UE3UE4
        //@Rock - JN - mod to remove expensive cost of map search for each texture in GetWantedMips2
        //   because we have to do some expensive searches here when removing levels this is in pool thread
        //   perhaps we should throttle the thread for a frame after this remove so
        // Remove old levels. Note: Don't try to access the ULevel object, it may have been deleted already!
 
        //@VIRTUOS [Steve Shen] : multi-thread protection, ThreadSettings.LevelData can be accessed by multiply threads(FAsyncTextureStreaming, game thread).
        FScopeLock Lock(&StreamingManager.ThreadSettingsLock);
 
        for ( int32 LevelIndex=0; LevelIndex < StreamingManager.ThreadSettings.LevelData.Num(); ++LevelIndex )
        {
            FStreamingManagerTexture::FLevelData* LevelData = StreamingManager.ThreadSettings.LevelData.GetData()[LevelIndex];
            if ( LevelData->Value.bRemove )
            {
                //@Rock - JN - slow, we have to update all the level indexes for each streaming texture instance :(
                const int32 NumStreamingTexture = StreamingManager.StreamingTextures.Num();
                for(int32 i=0; i<NumStreamingTexture; ++i)
                {
                    TArray<FStreamingTexture::TextureInstanceInfoData> & ThreadTextureInstancesData = StreamingManager.StreamingTextures[i].ThreadTextureInstancesData;
                    for(int32 j=0; j<ThreadTextureInstancesData.Num(); ++j)
                    {
                        if(ThreadTextureInstancesData[j].LevelData == LevelData)
                        {
                            ThreadTextureInstancesData.RemoveAtSwap(j--);
                        }  
                    }
                }
 
                delete StreamingManager.ThreadSettings.LevelData[LevelIndex];
 
                StreamingManager.ThreadSettings.LevelData.RemoveAtSwap(LevelIndex--);
            }
        }
#endif     
        PrioritizedTextures.Empty( StreamingManager.StreamingTextures.Num() );
...
//      StreamingManager.CalcDynamicWantedMips();
        ...
        for ( int32 Index=0; Index < StreamingManager.StreamingTextures.Num() && !IsAborted(); ++Index )
        {
            ...
            if ( StreamingTexture.bReadyForStreaming )
            {
                ...
                int32 l_PreviousWantedMips = StreamingTexture.WantedMips;
                StreamingManager.CalcWantedMips( StreamingTexture );
 
                // @VIRTUOS [BG] BEGIN: Add measure about the WantedMips
                if(StreamingTexture.Texture && (StreamingTexture.WantedMips != l_PreviousWantedMips))
                {
                    const TIndirectArray<FTexture2DMipMap>& OwnerMips = StreamingTexture.Texture->GetPlatformMips();
                    if(OwnerMips.Num() > 0)
                    {
                        VMeasure Measure(VMeasure::LY_TextureStreaming, VMeasure::ST_WantedMipsUpdate, *(OwnerMips[0].BulkData.GetFilename()), VMeasure::DT_MipLevel);
                        Measure.SetOffset(l_PreviousWantedMips);
                        Measure.SetCount(StreamingTexture.WantedMips);
                    }
                }
                // @VIRTUOS [BG] END
 ...
 */
void IStreamingManager::AddViewInfoToArray( TArray<FStreamingViewInfo> &ViewInfos, const FVector& ViewOrigin, float ScreenSize, float FOVScreenSize, const FPlane & viewPlane, float BoostFactor, bool bOverrideLocation, float Duration, TWeakObjectPtr<AActor> InActorToBoost )
{
    ...
    for ( int32 ViewIndex=0; ViewIndex < ViewInfos.Num(); ++ViewIndex )
    {
            ...
            ViewInfo.bOverrideLocation == bOverrideLocation )
        {
            ...
            ViewInfo.ViewPlane = viewPlane; //@Rock - JG: Attempt to improve texture streaming code.
        }
    }
 
    if ( bShouldAddView )
    {
        new (ViewInfos) FStreamingViewInfo( ViewOrigin, ScreenSize, FOVScreenSize, BoostFactor, bOverrideLocation, Duration, InActorToBoost, viewPlane );
    }
}
...
void IStreamingManager::SetupViewInfos( float DeltaTime )
{
    ...
    for ( int32 SlaveLocationIndex=0; SlaveLocationIndex < SlaveLocations.Num(); SlaveLocationIndex++ )
    {
        ...
        AddViewInformation( SlaveLocation.Location, ScreenSize, FOVScreenSize, ViewPlane , SlaveLocation.BoostFactor, SlaveLocation.bOverrideLocation, SlaveLocation.Duration);
    }
    ...
    for ( int32 ViewIndex=0; ViewIndex < LastingViewInfos.Num(); ++ViewIndex )
    {
        ...
        {
            AddViewInfoToArray( CurrentViewInfos, ViewInfo.ViewOrigin, ViewInfo.ScreenSize * SplitScreenFactor, ViewInfo.FOVScreenSize, ViewPlane , ViewInfo.BoostFactor, ViewInfo.bOverrideLocation, ViewInfo.Duration, ViewInfo.ActorToBoost);
        }
    }
 ...
 * @param InActorToBoost        Optional pointer to an actor who's textures should have their streaming priority boosted
 */
void IStreamingManager::AddViewInformation( const FVector& ViewOrigin, float ScreenSize, float FOVScreenSize, const FPlane & viewPlane , float BoostFactor/*=1.0f*/, bool bOverrideLocation/*=false*/, float Duration/*=0.0f*/, TWeakObjectPtr<AActor> InActorToBoost /*=NULL*/) //@Rock - JG: Attempt to improve texture streaming code with viewPlane.
{
    ...
    if ( FMath::Abs(ViewOrigin.X) < (1.0e+20f) && FMath::Abs(ViewOrigin.Y) < (1.0e+20f) && FMath::Abs(ViewOrigin.Z) < (1.0e+20f) )
    {
        ...
        if ( Duration > 0.0f )
        {
            AddViewInfoToArray( LastingViewInfos, ViewOrigin, ScreenSize, FOVScreenSize, viewPlane , BoostFactor, bOverrideLocation, Duration, InActorToBoost);
        }
        else
        {
            AddViewInfoToArray( PendingViewInfos, ViewOrigin, ScreenSize, FOVScreenSize, viewPlane , BoostFactor, bOverrideLocation, 0.0f, InActorToBoost);
        }
    }
    else
    {
...
int32 FStreamingManagerCollection::BlockTillAllRequestsFinished( bool &bNoMoreHighPriority, bool FlushRenderingCommands, float TimeLimit /*= 0.0f*/, bool bLogResults /*= false*/)
{
    return 0;
}
...
{
    //@ROCK - Dustin: Show a loading symbol
    /*if( GLoadingIconSupport && TimeLimit >= 4.0f )
    {
        GLoadingIconSupport->GameThreadStartLoadingIcon();
    }*/
    ...
    //@ROCK - Dustin: Hide the loading symbol
    /*if( GLoadingIconSupport && TimeLimit != 0 )
    {
        GLoadingIconSupport->GameThreadStopLoadingIcon();
    }*/
...
//@VIRTUOS [TV]
bool FStreamingManagerCollection::IsLevelPending(class ULevel* Level)
{
    bool result = false;
    for( int32 ManagerIndex=0; ManagerIndex<StreamingManagers.Num(); ManagerIndex++ )
    {
        IStreamingManager* StreamingManager = StreamingManagers[ManagerIndex];
        if (StreamingManager->IsLevelPending(Level))
        {
            result = true;
        }
    }
...
/**
 * Called when a MIC has had its textured changed.
 * Search during primitives that were already attached.
 * Get primitives using this MIC and update it.
 */
void FStreamingManagerCollection::NotifyMICUpdated( class UMaterialInterface* MIC )
{
    STAT( double StartTime = FPlatformTime::Seconds() );
    ...
    if (MIC)
    {
        ...
        {
            IStreamingManager* StreamingManager = StreamingManagers[ManagerIndex];
            StreamingManager->NotifyMICUpdated( MIC );
        }
    }
    STAT( GStreamingDynamicPrimitivesTime += FPlatformTime::Seconds() - StartTime );
}
...
#endif
                //@VIRTUOS [Steve Shen] : prevent Texture2D UpdateResource during CacheUniformExpressions in render thread.
                FScopeLock lock(&UTexture2D::Texture2DLock);
                for( TObjectIterator<UTexture2D>It; It; ++It )
                {
#if PLATFORM_XBOXONE
                    // Virtuos SHL: Call ProcessEvents when do initialization to process suspend event
                    if(static_cast<float>(FPlatformTime::Seconds() - StartTime) > 0.2f)
                    {
                        gCallProcessEvents();
...
,   bPauseTextureStreaming(false)
{
    ...
    // @Virtuos [DP] In Xbox360 Memory marge is set at 16, in your version is set to 20. I forced it at 160 Because we have multiplied by 10 texture pool size.
    //  When this value is at 20 we have unwanted waiting in StreamTexture.
    MemoryMargin = 100;
    ...
    {
        const FTextureLODGroup& TexGroup = UDeviceProfileManager::Get().GetActiveProfile()->GetTextureLODSettings()->GetTextureLODGroup(TextureGroup(LODGroup), TEXTURESUBGROUP_None);
        ThreadSettings.NumStreamedMips[LODGroup] = TexGroup.NumStreamedMips;
    }
    for ( int32 LODSubGroup=0; LODSubGroup < TEXTURESUBGROUP_MAX - 1; ++LODSubGroup )
    {
        const FTextureLODGroup& TexGroup = UDeviceProfileManager::Get().GetActiveProfile()->GetTextureLODSettings()->GetTextureLODGroup(TEXTUREGROUP_Lightmap, TextureSubGroup(LODSubGroup));
        ThreadSettings.LightmapNumStreamedMips[LODSubGroup] = TexGroup.NumStreamedMips;
    }
    for ( int32 LODSubGroup=0; LODSubGroup < TEXTURESUBGROUP_MAX - 1; ++LODSubGroup )
    {
        const FTextureLODGroup& TexGroup = UDeviceProfileManager::Get().GetActiveProfile()->GetTextureLODSettings()->GetTextureLODGroup(TEXTUREGROUP_Shadowmap, TextureSubGroup(LODSubGroup));
        ThreadSettings.ShadowmapNumStreamedMips[LODSubGroup] = TexGroup.NumStreamedMips;
    }
...
bool FStreamingManagerTexture::StreamOutTextureData( int64 RequiredMemorySize )
{
    ...
    for (int32 StreamingIndex = 0; StreamingIndex < StreamingTextures.Num(); ++StreamingIndex)
    {
        ...
        if (StreamingTexture.Texture)
        {
            ...
            // @VIRTUOS [BG] Get resident mip count based on Mip data
            NumRequiredResidentMips = FMath::Max<int32>(NumRequiredResidentMips, FMath::Max(UTexture2D::GetMinTextureResidentMipCount(Texture->GetPlatformMips(),Texture->LODGroup,Texture->LODSubGroup), UDeviceProfileManager::Get().GetActiveProfile()->GetTextureLODSettings()->GetMinLODMipCount(Texture->LODGroup, Texture->LODSubGroup))); //@ROCK - Dustin: Added support for per LOD group min texture resident mip count
            //NumRequiredResidentMips = FMath::Max<int32>(NumRequiredResidentMips, FMath::Max(UTexture2D::GetMinTextureResidentMipCount(), UDeviceProfileManager::Get().GetActiveProfile()->GetTextureLODSettings()->GetMinLODMipCount(Texture->LODGroup, Texture->LODSubGroup))); //@ROCK - Dustin: Added support for per LOD group min texture resident mip count          
...
{
    //@VIRTUOS [Steve Shen] : multi-thread protection, PendingStreamingTextures can be accessed by multiply threads(FAsyncLoadingThread, game thread).
    {
        FScopeLock Lock(&PendingStreamingTexturesLock);
    ...
    //@VIRTUOS [Steve Shen] : multi-thread protection, ThreadSettings.LevelData can be accessed by multiply threads(FAsyncTextureStreaming, game thread).
    FScopeLock Lock(&ThreadSettingsLock);
...
#if VIRTUOS_PAIR_UE3UE4
        FLevelData* LevelData = new FLevelData(Level);
        ThreadSettings.LevelData.Add(LevelData);
#else
...
#if VIRTUOS_PAIR_UE3UE4
        SwapArrays(ThreadLevelData.BoundingSpheres, Level->BoundingSpheres);
        ThreadLevelData.BoundingSpheres.Shrink();
#else
        ThreadLevelData.TextureInstancesArray.Reset();
#endif
...
#if VIRTUOS_PAIR_UE3UE4
                uint32 ThreadTextureInstancesIndex = ThreadLevelData.TextureInstancesArray.Add(TArray<FStreamableTextureInstance>());
 
                // swap the ULevel with the threaded ones to avoid copy
                SwapArrays(ThreadLevelData.TextureInstancesArray[ThreadTextureInstancesIndex], TextureInstances);
                ThreadLevelData.TextureInstancesArray[ThreadTextureInstancesIndex].Shrink();
                StreamingTexture.AddTextureInstanceInfo(LevelData, ThreadTextureInstancesIndex);
#else
                TArray<FStreamableTextureInstance4>& TextureInstances4 = ThreadLevelData.TextureInstancesArray.Add( Texture2D, TArray<FStreamableTextureInstance4>() );
                //uint32 ThreadTextureInstancesIndex = ThreadLevelData.TextureInstancesArray.Add(TArray<FStreamableTextureInstance4>());
                //TArray<FStreamableTextureInstance4>& TextureInstances4 = ThreadLevelData.TextureInstancesArray(ThreadTextureInstancesIndex);
                //TArray<FStreamableTextureInstance4>& TextureInstances4 = ThreadLevelData.TextureInstancesArray[ThreadTextureInstancesIndex];
...
bool FStreamingManagerTexture::IsLevelPending(ULevel* Level)
{
    bool result = false;
    for (int i = 0; i < PendingLevels.Num(); i++)
    {
        ULevel* level = PendingLevels[i];
        if (level == Level)
            result = true;
    }
    return result;
...
void FStreamingManagerTexture::RemoveLevel( ULevel* Level )
{
    ...
    //@Rock - JN - mod to remove expensive cost of map search for each texture in GetWantedMips2
    for(int32 i=0; i<ThreadSettings.LevelData.Num(); ++i)
    {
        FLevelData* LevelData = ThreadSettings.LevelData[i];
 ...
 * Get primitives using this MIC and update it.
 */
void FStreamingManagerTexture::NotifyMICUpdated( class UMaterialInterface* MIC )
{
    for ( TMap<const UPrimitiveComponent*,FSpawnedPrimitiveData>::TIterator It(ThreadSettings.SpawnedPrimitives); It; ++It )
    {
        const UPrimitiveComponent* Primitive = It.Key();
        ...
        if ( Primitive && PrimitiveData.bAttached && !PrimitiveData.bPendingUpdate)
        {
            const UMeshComponent* MeshComponent = Cast<UMeshComponent>(Primitive);
            if (MeshComponent && MeshComponent->OverrideMaterials.Find(MIC) != INDEX_NONE)
            {
                NotifyPrimitiveUpdated(MeshComponent);
            }
        }
    }
}
...
//  TotalIntermediateTexturesSize           += Context.ThisFrameTotalIntermediateTexturesSize;
    ...
    if ( bAllTexturesProcessed )
    {
        ...
        SET_DWORD_STAT( STAT_StreamingTextures_Custom,              NumStreamingTextures            );
        SET_MEMORY_STAT( STAT_StreamingTexturesSize_Custom,     TotalStreamingTexturesSize      );
        SET_MEMORY_STAT( STAT_OptimalTextureSize_Custom,            TotalOptimalWantedSize          );
...
//@Virtuos [Steve Shen] : BEGIN : integrate Streaming related FPS optimizations
enum StreamTexturesStages
{
    eSTREAM_TEXTURES_STAGES_INIT = 0,
    eSTREAM_TEXTURES_STAGES_PROCESS,
    eSTREAM_TEXTURES_STAGES_FINISH
};
 
bool FStreamingManagerTexture::StreamTextures( bool bProcessEverything )
{
    TM_SCOPE_WITH_RENDER("StreamTextures");
     
    //@Virtuos [Steve Shen] : BEGIN : integrate Streaming related FPS optimizations
    bool bDone = false;
 
    static StreamTexturesStages currentStage = eSTREAM_TEXTURES_STAGES_INIT;
    static int64 AvailableNow = 0;
    static int64 AvailableLater = 0;
    static int64 TempMemoryUsed = 0;
 
    static bool bIsUsingTexturePool = false;;
 
    static int32 HighPrioIndex = 0;
    static int32 LowPrioIndex = 0;
    static int32 LowPrioIndex_UnwantedMips = 0;
    //@Virtuos [Steve Shen] : END
    ...
    if (currentStage == eSTREAM_TEXTURES_STAGES_INIT)       //@Virtuos [Steve Shen] : integrate Streaming related FPS optimizations
    {
        ...
        //int64 AvailableNow, AvailableLater, TempMemoryUsed;       //@Virtuos [Steve Shen] : integrate Streaming related FPS optimizations
        if ( Stats.IsUsingLimitedPoolSize() )
        {
            ...
            SET_MEMORY_STAT( STAT_StreamingOverBudget_Custom,   FMath::Max(MemoryOverBudget,0ll) );
            SET_MEMORY_STAT( STAT_NonStreamingTexturesSize_Custom,  FMath::Max<int64>(NonStreamingUsage, 0) );
            bIsUsingTexturePool = true;     //@Virtuos [Steve Shen] : integrate Streaming related FPS optimizations
        }
        else
        {
            TempMemoryUsed = ThreadStats.TempStreamingSize;
            ...
            SET_MEMORY_STAT( STAT_StreamingOverBudget_Custom, 0 );
        }
        ...
        SET_DWORD_STAT( STAT_NumWantingTextures_Custom, ThreadStats.NumWantingTextures );
 
        //@Virtuos [Steve Shen] : integrate Streaming related FPS optimizations
        HighPrioIndex = 0;
        LowPrioIndex = PrioritizedTextures.Num() - 1;
 
        LowPrioIndex_UnwantedMips = LowPrioIndex;
        // Jump to the process phase
        currentStage = eSTREAM_TEXTURES_STAGES_PROCESS;
    } // if eSTREAM_TEXTURES_STAGES_INIT
    //@Virtuos [Steve Shen] : END
 
    if(currentStage == eSTREAM_TEXTURES_STAGES_PROCESS)     //@Virtuos [Steve Shen] : integrate Streaming related FPS optimizations
    {
        if ( bIsUsingTexturePool && !bPauseTextureStreaming )
        {
            ...
            const int maxTexStreamingOperationsPerFrame = 15;
            int numTexStreamingOperations = 0;
 
            bool bWorkRemaining = HighPrioIndex <= LowPrioIndex && TempMemoryUsed < MaxTempMemoryUsed;
 
            // @VIRTUOS [BG] BEGIN : Cancel texture streaming
        #if VIRTUOS_PAIR_UE3UE4
            for ( int32 PrioIndex=0; PrioIndex < PrioritizedTextures.Num() && TempMemoryUsed < MaxTempMemoryUsed; ++PrioIndex )
            {
                ...
                // Has this texture been marked for removal or nto yet sent request?
                if ( StreamingTexture.Texture == NULL || !StreamingTexture.bInFlight )
                {
                    continue;
                }
                ...
                if ( StreamingTexture.RequestedMips > StreamingTexture.ResidentMips && StreamingTexture.ResidentMips > StreamingTexture.WantedMips )
                {
                ...
                else if ( StreamingTexture.RequestedMips < StreamingTexture.ResidentMips && StreamingTexture.RequestedMips < StreamingTexture.WantedMips )
                {
                    int64 RequestedSize = StreamingTexture.GetSize( StreamingTexture.RequestedMips );
                    int64 StreamSize = StreamingTexture.GetSize( StreamingTexture.ResidentMips ) - RequestedSize;
                ...
                else if ( !StreamingTexture.Texture->bHasCancelationPending &&  StreamingTexture.bInFlight)
                {
                    ChangePriorityIfNeeded(StreamingTexture,TexturePriority.Priority >= HIGH_TEXTURE_PRIORITY);
                }
            }
        #endif
            // @VIRTUOS [BG] END
 
            //@VIRTUOS[XR]: Disable maxTexStreamingOperationsPerFrame optimization to use rather the Rocksteady one with MAX_TEXTURE_LOAD_REQUESTS
            while ( bWorkRemaining)// && (numTexStreamingOperations < maxTexStreamingOperationsPerFrame || bProcessEverything) )
            {
                ...
                if ( HighPrioTexture.Texture != NULL )
                {
                    ...
                    if ( HighPrioTexture.bInFlight == false && HighPrioTexture.WantedMips > HighPrioTexture.ResidentMips
                        && (bProcessEverything || NumTexturesLoading < MAX_TEXTURE_LOAD_REQUESTS ) ) //@ROCK - Dustin: When the IO queue gets too full, we don't want to process more increase requests
                    {
                        ...
                        if ( StreamSize <= AvailableNow && TempMemoryUsed < MaxTempMemoryUsed )
                        {
                            ...
                            StartStreaming( HighPrioTexture, HighPrioTexture.WantedMips, Context, true , TexturePriority.Priority >= HIGH_TEXTURE_PRIORITY );
                            numTexStreamingOperations++;    //@Virtuos [Steve Shen] : integrate Streaming related FPS optimizations
                        }
                    }
#if VIRTUOS_PAIR_UE3UE4 && !PLATFORM_PS4
                    // Would we like to stream in this texture?
                    else if ( !HighPrioTexture.Texture->bHasCancelationPending &&
                        HighPrioTexture.WantedMips > HighPrioTexture.RequestedMips
                        && (bProcessEverything || NumTexturesLoading < MAX_TEXTURE_LOAD_REQUESTS ) &&
                        HighPrioTexture.Texture->PendingMipChangeRequestStatus.GetValue() >= TexState_InProgress_Loading &&
                        HighPrioTexture.Texture->PendingMipChangeRequestStatus.GetValue() < TexState_InProgress_LoadingMoreMin)
                    {
                        //@TODO: TempSize=0 when using in-place reallocation, but we're conservative here.
                        int64 TempSize = HighPrioTexture.GetSize( HighPrioTexture.RequestedMips );
                        int64 StreamSize = HighPrioTexture.GetSize( HighPrioTexture.WantedMips ) - TempSize;
                        AvailableLater -= StreamSize;
                ...
                HighPrioIndex++;
                bWorkRemaining = HighPrioIndex <= LowPrioIndex && TempMemoryUsed < MaxTempMemoryUsed; //@Virtuos [Steve Shen] : integrate Streaming related FPS optimizations
            }
            ...
            if (!bWorkRemaining)
            {
                currentStage = eSTREAM_TEXTURES_STAGES_FINISH;
            }
        }
    ...
    //@Virtuos [Steve Shen] : integrate Streaming related FPS optimizations
    if(currentStage == eSTREAM_TEXTURES_STAGES_FINISH)
    {
        ...
        // Reset back to top
        currentStage = eSTREAM_TEXTURES_STAGES_INIT;
...
namespace UE3
{
    extern bool bIsUE3LoadingMoviePlaying;
}
...
{
//#if VIRTUOS_PAIR_UE3UE4
//  FScopeLock Lock(&m_SynchronizationObject);
//#endif
...
#if THROTTLE_UPDATE_MIPS
        bool bFlushTextureStreamingRequests = bProcessEverything || UE3::bIsUE3LoadingMoviePlaying;
#else
        bool bFlushTextureStreamingRequests = true;
#endif
         
        // If we are done sending all the streaming requests then reset back to the first stage.
        if (StreamTextures( bFlushTextureStreamingRequests ))
        {
            ProcessingStage = 0;
        }
    }
...
//@VIRTUOS [TV] Used to force streaming of texture before adding a level to the world. Returns the current processing stage.
int32 FStreamingManagerTexture::GetProcessingStage()
{
    return ProcessingStage;
}
 
//@VIRTUOS [TV] Used to force streaming of texture before adding a level to the world. Returns the number of texture processing stage.
int32 FStreamingManagerTexture::GetNumTextureProcessingStages()
{
    return NumTextureProcessingStages;
}
 
 
//@VIRTUOS [TV] Used to force streaming of lightmap before adding a level to the world.
void FStreamingManagerTexture::BoostLevelTextureStreaming(UTexture2D * texture, float boostFactor)
{
    if (texture && texture->StreamingIndex >= 0 && texture->StreamingIndex < StreamingTextures.Num())
    {
        StreamingTextures[ texture->StreamingIndex ].BoostFactor = boostFactor;
    }
}
...
int32 FStreamingManagerTexture::BlockTillAllRequestsFinished( bool &bNoMoreHighPriority, bool flushRenderingCommands, float TimeLimit /*= 0.0f*/, bool bLogResults /*= false*/ )
{
    double StartTime = FPlatformTime::Seconds();
...
#if PLATFORM_XBOXONE
    double StartTimeForSystemEvents = FPlatformTime::Seconds();
#endif
    do
    {      
#if VIRTUOS_PAIR_UE3UE4
        if (flushRenderingCommands)
#endif
        {
...
#if PLATFORM_XBOXONE
        // Virtuos SHL: Call ProcessEvents when in flush level streaming to process suspend event
        if(static_cast<float>(FPlatformTime::Seconds() - StartTimeForSystemEvents) > 0.2f)
        {
            gCallProcessEvents();
            StartTimeForSystemEvents = FPlatformTime::Seconds();
        }
#endif
 
        if ( NumPendingUpdates )
        {
            FPlatformProcess::Sleep( TimeLimit > 0 ? 0.002f : 0.010f );
        }
...
#if VIRTUOS_PAIR_UE3UE4
    bNoMoreHighPriority = true;
    if (NumPendingUpdates > 0)
    {
        for ( int32 PrioIndex=0; PrioIndex < PendingTextures[CurrentArray].Num(); PrioIndex++ )
        {
            if (PrioIndex < PendingTextures[CurrentArray].Num() && PrioIndex >= 0)
            {
                int32 TexturePrioIndex = PendingTextures[CurrentArray][ PrioIndex ];
 
                if (TexturePrioIndex < StreamingTextures.Num() && TexturePrioIndex > 0)
                {
                    FStreamingTexture& StreamingTexture = StreamingTextures[ TexturePrioIndex ];
                    if (StreamingTexture.bMAXPriority)
                        bNoMoreHighPriority = false;
                }
            }
        }
    }
...
{
    bool dummy = false;
    return BlockTillAllRequestsFinished( dummy, true, TimeLimit /*= 0.0f*/, bLogResults /*= false*/);
}
...
void FStreamingManagerTexture::CalcMinMaxMips( FStreamingTexture& StreamingTexture )
{
    ...
    // @VIRTUOS [BG] Get resident mip count based on Mip data, be careful if texture isn't set on Streaming Texture default resident mipcount is used
    UTexture2D::CalcAllowedMips(
        ...
        StreamingTexture.MaxAllowedMips,
        StreamingTexture.Texture ? UTexture2D::GetMinTextureResidentMipCount(StreamingTexture.Texture->GetPlatformMips(),StreamingTexture.LODGroup,StreamingTexture.LODSubGroup) :
    ...
    else
    {
        ...
        if(StreamingTexture.LODGroup == TEXTUREGROUP_Lightmap && StreamingTexture.LODSubGroup != TEXTURESUBGROUP_None)
            NumStreamedMips = ThreadSettings.LightmapNumStreamedMips[StreamingTexture.LODSubGroup - 1];
        if(StreamingTexture.LODGroup == TEXTUREGROUP_Shadowmap && StreamingTexture.LODSubGroup != TEXTURESUBGROUP_None)
            NumStreamedMips = ThreadSettings.ShadowmapNumStreamedMips[StreamingTexture.LODSubGroup - 1];
        else
            NumStreamedMips = ThreadSettings.NumStreamedMips[StreamingTexture.LODGroup];
        if ( NumStreamedMips >= 0 )
            StreamingTexture.MinAllowedMips = FMath::Clamp( StreamingTexture.MipCount - NumStreamedMips, StreamingTexture.MinAllowedMips, StreamingTexture.MaxAllowedMips );
    }
...
/** @Virtuos [DP]
    *   Test if a change of priority is needed and request it.
    *  
    *   @param StreamingTexture         Texture to change priority 
    *   @return                         true if texture is now in change priority
    */
bool FStreamingManagerTexture::ChangePriorityIfNeeded(FStreamingTexture& StreamingTexture, bool bHighPriorityTextureRequest /*= false*/)
{
    UTexture2D* Texture = StreamingTexture.Texture;
    FTexture2DResource* Texture2DResource = (FTexture2DResource*) Texture->Resource;
     
    if( (StreamingTexture.bForceFullyLoad || bHighPriorityTextureRequest) != (StreamingTexture.bPreviousPriority) &&
        Texture->PendingMipChangeRequestStatus.GetValue() >= TexState_InProgress_Loading &&
        Texture->PendingMipChangeRequestStatus.GetValue() < TexState_InProgress_LoadingMoreMin)
    {
        Texture2DResource->BeginUpdateRequestPriority(StreamingTexture.bForceFullyLoad || bHighPriorityTextureRequest);
        StreamingTexture.bPreviousPriority = StreamingTexture.bForceFullyLoad || bHighPriorityTextureRequest;
        return true;
    }
 ...
 */
bool FStreamingManagerTexture::StartStreaming( FStreamingTexture& StreamingTexture, int32 RequestMips, FStreamingContext& Context, bool bIgnoreMemoryConstraints, bool bHighPriorityTextureRequest )
{
    UTexture2D* Texture = StreamingTexture.Texture;
    FTexture2DResource* Texture2DResource = (FTexture2DResource*) Texture->Resource;
    // @Virtuos [XuFan] BEGIN:
    if (GIsBatchImportingLevels && !Texture2DResource)
    {
        return false;
    }
    // @Virtuos END
    ...
    if ( RequestMips != StreamingTexture.ResidentMips && StreamingTexture.bReadyForStreaming )
    {
        ...
        {
            int32 l_PreviousRequestedMips = StreamingTexture.RequestedMips;
 
            if (!StreamingTexture.bInFlight && Texture->PendingMipChangeRequestStatus.GetValue() == TexState_ReadyFor_Requests &&
                 RequestMips != StreamingTexture.Texture->ResidentMips )
            {
                ...
                // @VIRTUOS [DP] it's a first request. We didn't have Previous request
                Texture->PreviousRequestedMips = RequestMips;
                ...
                bool bShouldPrioritizeAsyncIORequest = StreamingTexture.bForceFullyLoad || bHighPriorityTextureRequest; //@ROCK - Dustin: Added support for different package priorities
                Texture2DResource->BeginUpdateMipCount( bShouldPrioritizeAsyncIORequest );
                StreamingTexture.bPreviousPriority = bShouldPrioritizeAsyncIORequest;
                StreamingTexture.bInFlight = true;
                TrackTextureEvent( &StreamingTexture, StreamingTexture.Texture, StreamingTexture.bForceFullyLoad, this );
 
                //@ROCK - Dustin: Keep track of how many requests we make, don't create more than we can service!
                NumTexturesLoading++;
            }
            // @Virtuos [DP] Change Request if biggest Mips is required
#if VIRTUOS_PAIR_UE3UE4 && !PLATFORM_WINDOWS
            else if(Texture->PendingMipChangeRequestStatus.GetValue() >= TexState_InProgress_Loading &&
                Texture->PendingMipChangeRequestStatus.GetValue() < TexState_InProgress_LoadingMoreMin &&
                RequestMips > StreamingTexture.Texture->RequestedMips)
            {
                // Manually update size as allocations are deferred/ happening in rendering thread.
                int64 CurrentRequestSize            = StreamingTexture.GetSize(RequestMips);
                int64 StreamInSize                  = CurrentRequestSize - StreamingTexture.GetSize(StreamingTexture.RequestedMips);
                Context.ThisFrameTotalRequestSize   += StreamInSize;
                Context.ThisFrameTotalLightmapRequestSize += StreamingTexture.bIsStreamingLightmap ? StreamInSize : 0;
                Context.AvailableMemorySize         -= CurrentRequestSize;
                ...
                check(!Texture->bHasCancelationPending);
                // Set new requested mip count.
                Texture->PreviousRequestedMips = Texture->RequestedMips;
                Texture->RequestedMips = RequestMips;
                StreamingTexture.RequestedMips = RequestMips;
 
                Texture2DResource->BeginUpgradeMipCount();
            ...
            // @VIRTUOS [BG] BEGIN: Add measure about the RequestedMips
            if(StreamingTexture.RequestedMips != l_PreviousRequestedMips)
            {
                const TIndirectArray<FTexture2DMipMap>& OwnerMips = StreamingTexture.Texture->GetPlatformMips();
                if(OwnerMips.Num() > 0)
                {
                    VMeasure Measure(VMeasure::LY_TextureStreaming, VMeasure::ST_RequestedMipsUpdate, *(OwnerMips[0].BulkData.GetFilename()), VMeasure::DT_MipLevel);
                    Measure.SetOffset(l_PreviousRequestedMips);
                    Measure.SetCount(StreamingTexture.RequestedMips);
                }
            }
            // @VIRTUOS [BG] END
        ...
        case TEXTUREGROUP_Shadowmap:
            {
                for ( int32 LODSubGroup = 0;  LODSubGroup < TEXTURESUBGROUP_MAX - 1; ++LODSubGroup )
                {
                    FTextureLODGroup& TexGroup = UDeviceProfileManager::Get().GetActiveProfile()->GetTextureLODSettings()->GetTextureLODGroup(TextureGroup(LODGroup), TextureSubGroup(LODSubGroup));
                    if ( NumMips >= -1 && NumMips <= MAX_TEXTURE_MIP_COUNT )
                    {
                        TexGroup.NumStreamedMips = NumMips;
                    }
                    Ar.Logf( TEXT("%s.NumStreamedMips = %d"), UTexture::GetTextureGroupString(TextureGroup(LODGroup), TextureSubGroup(LODSubGroup + 1)), TexGroup.NumStreamedMips );
                }
            }
            // break;
        default:
            {
                FTextureLODGroup& TexGroup = UDeviceProfileManager::Get().GetActiveProfile()->GetTextureLODSettings()->GetTextureLODGroup(TextureGroup(LODGroup), TEXTURESUBGROUP_None);
                if ( NumMips >= -1 && NumMips <= MAX_TEXTURE_MIP_COUNT )
                {
                    TexGroup.NumStreamedMips = NumMips;
                }
                Ar.Logf( TEXT("%s.NumStreamedMips = %d"), UTexture::GetTextureGroupString(TextureGroup(LODGroup), TEXTURESUBGROUP_None), TexGroup.NumStreamedMips );
            }
        }
                ...
                for ( int32 TrackedTextureIndex=0; TrackedTextureIndex < NumTrackedTextures; ++TrackedTextureIndex )
                {
                    ...
                    if ( FCString::Stristr(TextureName, *TrackedTextureName) != NULL )
                    {
                        ...
                        for ( int32 LevelIndex=0; LevelIndex < ThreadSettings.LevelData.Num(); ++LevelIndex )
                        {
                            ...
                            /*int32 NumTextureInstances = StreamingTexture.GetNumTextureInstances();
                            int32 TextureInstancesIndex = -1;
 
                            for(int32 i=0; i<NumTextureInstances; ++i)
                            {
                            TextureInstancesIndex = StreamingTexture.GetTextureInstancesIndex(i);
                            }*/
#else
                            FStreamingManagerTexture::FThreadLevelData& LevelData = ThreadSettings.LevelData[ LevelIndex ].Value;
                            TArray<FStreamableTextureInstance4>* TextureInstances = LevelData.TextureInstancesArray.Find( StreamingTexture.Texture );
                            if ( TextureInstances )
                            {
                                for ( int32 InstanceIndex=0; InstanceIndex < TextureInstances->Num(); ++InstanceIndex )
                                {
...
bool FStreamingManagerTexture::HandleStreamingManagerMemoryCommand( const TCHAR* Cmd, FOutputDevice& Ar, UWorld* InWorld )
{
    ...
    for ( int32 LevelIndex=0; LevelIndex < ThreadSettings.LevelData.Num(); ++LevelIndex )
    {
        ...
        for ( TMap<const UTexture2D*,TArray<FStreamableTextureInstance4> >::TConstIterator It(LevelData.Value.TextureInstancesArray); It; ++It )
        {
...
void FStreamingManagerTexture::DumpTextureGroupStats( bool bDetailedStats )
{
    ...
    FTextureGroupStats LightmapSubGroupStats[TEXTURESUBGROUP_MAX];
    FTextureGroupStats LightmapSubGroupWaste[TEXTURESUBGROUP_MAX];
    FTextureGroupStats ShadowmapSubGroupStats[TEXTURESUBGROUP_MAX];
    FTextureGroupStats ShadowmapSubGroupWaste[TEXTURESUBGROUP_MAX];
...
//  int32 UITexels = 0;
    ...
    for( TObjectIterator<UTexture> It; It; ++It )
    {
        ...
        if(Texture->LODGroup == TEXTUREGROUP_Lightmap && Texture->LODSubGroup != TEXTURESUBGROUP_None)
        {
            Stat = LightmapSubGroupStats[Texture->LODSubGroup - 1];
            Waste = LightmapSubGroupWaste[Texture->LODSubGroup - 1];
        }
        else if(Texture->LODGroup == TEXTUREGROUP_Shadowmap && Texture->LODSubGroup != TEXTURESUBGROUP_None)
        {
            Stat = ShadowmapSubGroupStats[Texture->LODSubGroup - 1];
            Waste = ShadowmapSubGroupWaste[Texture->LODSubGroup - 1];
        }
...
//
    ...
    // Output stats.
    {
        ...
        for ( int32 GroupIndex=0; GroupIndex < TEXTUREGROUP_MAX; ++GroupIndex )
        {
            ...
            UE_LOG(LogContentStreaming, Log, TEXT("%34s: NumTextures=%4d, Current=%8.1f KB, Wanted=%8.1f KB, OnDisk=%8.1f KB, NumNonStreaming=%4d, NonStreaming=%8.1f KB"),
                UTexture::GetTextureGroupString((TextureGroup)GroupIndex, TEXTURESUBGROUP_None),
                Stat.NumTextures,
        ...
        for ( int32 GroupIndex=0; GroupIndex < TEXTURESUBGROUP_MAX - 1; ++GroupIndex )
        {
            FTextureGroupStats& Stat = LightmapSubGroupStats[GroupIndex];
            TotalStats.NumTextures              += Stat.NumTextures;
            TotalStats.NumNonStreamingTextures  += Stat.NumNonStreamingTextures;
            TotalStats.CurrentTextureSize       += Stat.CurrentTextureSize;
            TotalStats.WantedTextureSize        += Stat.WantedTextureSize;
            ...
            TotalStats.NonStreamingSize         += Stat.NonStreamingSize;
            UE_LOG(LogContentStreaming, Log, TEXT("%34s: NumTextures=%4d, Current=%8.1f KB, Wanted=%8.1f KB, OnDisk=%8.1f KB, NumNonStreaming=%4d, NonStreaming=%8.1f KB"),
                UTexture::GetTextureGroupString(TEXTUREGROUP_Lightmap, (TextureSubGroup)(GroupIndex + 1)),
                Stat.NumTextures,
                Stat.CurrentTextureSize / 1024.0f,
                Stat.WantedTextureSize / 1024.0f,
                Stat.MaxTextureSize / 1024.0f,
        ...
        for ( int32 GroupIndex=0; GroupIndex < TEXTURESUBGROUP_MAX - 1; ++GroupIndex )
        {
            FTextureGroupStats& Stat = ShadowmapSubGroupStats[GroupIndex];
            TotalStats.NumTextures              += Stat.NumTextures;
            TotalStats.NumNonStreamingTextures  += Stat.NumNonStreamingTextures;
            TotalStats.CurrentTextureSize       += Stat.CurrentTextureSize;
            TotalStats.WantedTextureSize        += Stat.WantedTextureSize;
            ...
            TotalStats.NonStreamingSize         += Stat.NonStreamingSize;
            UE_LOG(LogContentStreaming, Log, TEXT("%34s: NumTextures=%4d, Current=%8.1f KB, Wanted=%8.1f KB, OnDisk=%8.1f KB, NumNonStreaming=%4d, NonStreaming=%8.1f KB"),
                UTexture::GetTextureGroupString(TEXTUREGROUP_Shadowmap, (TextureSubGroup)(GroupIndex + 1)),
                Stat.NumTextures,
                Stat.CurrentTextureSize / 1024.0f,
                Stat.WantedTextureSize / 1024.0f,
                Stat.MaxTextureSize / 1024.0f,
    ...
    if ( bDetailedStats )
    {
        ...
        for ( int32 GroupIndex=0; GroupIndex < TEXTURESUBGROUP_MAX - 1; ++GroupIndex )
        {
            FTextureGroupStats& Stat = LightmapSubGroupWaste[GroupIndex];
            TotalStats.NumTextures              += Stat.NumTextures;
            TotalStats.NumNonStreamingTextures  += Stat.NumNonStreamingTextures;
            TotalStats.CurrentTextureSize       += Stat.CurrentTextureSize;
            TotalStats.WantedTextureSize        += Stat.WantedTextureSize;
        ...
        for ( int32 GroupIndex=0; GroupIndex < TEXTURESUBGROUP_MAX - 1; ++GroupIndex )
        {
            FTextureGroupStats& Stat = ShadowmapSubGroupWaste[GroupIndex];
            TotalStats.NumTextures              += Stat.NumTextures;
            TotalStats.NumNonStreamingTextures  += Stat.NumNonStreamingTextures;
            TotalStats.CurrentTextureSize       += Stat.CurrentTextureSize;
            TotalStats.WantedTextureSize        += Stat.WantedTextureSize;
...
void FStreamingManagerTexture::InvestigateTexture( const FString& InInvestigateTextureName )
{
    ...
    for ( int32 TextureIndex=0; TextureIndex < StreamingTextures.Num(); ++TextureIndex )
    {
        ...
        if (TextureName.Contains(InInvestigateTextureName))
        {
            ...
            UE_LOG(LogContentStreaming, Log,  TEXT("  Texture group:   %s"), UTexture::GetTextureGroupString(StreamingTexture.LODGroup, StreamingTexture.LODSubGroup) );
...
#if ENABLE_BICUBIC_UPSCALING
                                            100 / GBicubicUpscalingPercentageX *
#endif                         
                                            ViewInfo.FOVScreenSize * ViewInfo.BoostFactor * ScreenSizeFactor;
    ...
    if( StreamingManager.ThreadNumViews() /*&& StreamingTexture.Instances.Num() > 0*/ )
    {
        ...
        switch ( StreamingTexture.LODGroup )
        {
            ...
            case TEXTUREGROUP_Shadowmap:
                ScreenSizeFactor = GShadowmapStreamingFactor;
                break;
                //@ROCK - Dustin: Allow tweaking of LOD texture texel sizes
            case TEXTUREGROUP_ProcBuilding_Face:
                ScreenSizeFactor = GNearLODStreamingFactor;
                break;
            case TEXTUREGROUP_ProcBuilding_LightMap:
                ScreenSizeFactor = GFarLODStreamingFactor;
                break;
            default:
                ScreenSizeFactor = 1.0f;
        }
        ...
        //@Rock - JN - the reason for all this - instead of searching with LevelData.TextureInstances.Find(read expensive)
        // store a list of level index and texture instance indexes for each streaming texture
#if VIRTUOS_PAIR_UE3UE4
        static TArray<FStreamableTextureInstance4> TextureInstances4; //@ROCK - BW: Made static to reduce memory churn
        ...
        for(int32 i=0; i<NumTextureInstances; ++i)
        {
            FStreamingManagerTexture::FThreadLevelData& LevelData = StreamingTexture.GetLevelData(i)->Value;
 
            const int32 TextureInstancesIndex = StreamingTexture.GetTextureInstancesIndex(i);
 
            TArray<FStreamableTextureInstance>* TextureInstances = &LevelData.TextureInstancesArray.GetData()[TextureInstancesIndex];
 
            if ( TextureInstances )
            {
                StreamingTexture.bUsesStaticHeuristics = true;
 
                // @ROck - JN - convert to simd format as we store in more memory saving format
                TextureInstances4.Reset( (TextureInstances->Num() + 3) / 4 );
                for ( int32 InstanceIndex=0; InstanceIndex < TextureInstances->Num(); ++InstanceIndex )
                {
                    if ( (InstanceIndex & 3) == 0 )
                    {
                    ...
                    const FStreamableTextureInstance& Instance = (*TextureInstances)[ InstanceIndex ];
                    FStreamableTextureInstance4& Instance4 = TextureInstances4[ InstanceIndex/4 ];
                    FSphere BoundingSphere = LevelData.BoundingSpheres[Instance.BoundingSphereIndex]; //@ROCK - BW: Changed BoundingSphere to a table lookup
                    Instance4.BoundingSphereX[ InstanceIndex & 3 ] = BoundingSphere.Center.X;
                    Instance4.BoundingSphereY[ InstanceIndex & 3 ] = BoundingSphere.Center.Y;
                    Instance4.BoundingSphereZ[ InstanceIndex & 3 ] = BoundingSphere.Center.Z;
                    Instance4.BoundingSphereRadius[ InstanceIndex & 3 ] = BoundingSphere.W;
                    Instance4.TexelFactor[ InstanceIndex & 3 ] = Instance.TexelFactor;
                }
 
                for ( int32 InstanceIndex=0; InstanceIndex < TextureInstances4.Num() && !bShouldAbortLoop; ++InstanceIndex )
                {
                    const FStreamableTextureInstance4& TextureInstance = TextureInstances4[InstanceIndex];
...
#if ENABLE_BICUBIC_UPSCALING
                        FVector2D nativeScreenSize;
                        if (GWorld && GWorld->GetGameViewport())
                        {
                            GWorld->GetGameViewport()->GetViewportSize(nativeScreenSize);
                        }                      
                        float nativeFOVScreenSize = nativeScreenSize.X/ViewInfo.ScreenSize * ViewInfo.FOVScreenSize;
                        const float ScreenSizeFloat =  FMath::Lerp(nativeFOVScreenSize,
                            nativeScreenSize.X,
                            FMath::Min(StreamingTexture.LastRenderTime/10.0f, 1.0f)) * ScreenSizeFactor;  //@ROCK - Dustin: Changed to use FOV Screen size, for better results when zoomed in/out
 
#else
                        const float ScreenSizeFloat =
                            FMath::Lerp(ViewInfo.FOVScreenSize, ViewInfo.ScreenSize, FMath::Min(StreamingTexture.LastRenderTime/10.0f, 1.0f)) * ScreenSizeFactor;  //@ROCK - Dustin: Changed to use FOV Screen size, for better results when zoomed in/out
#endif
                ...
                if ( MinDistanceSq > 1.0f )
                {
                    ...
                    // WantedMipCount is the number of mips so we need to adjust with "+ 1".
                    WantedMipCount = FFloatMipLevel::FromScreenSizeInTexels(ScreenSizeInTexels);
                    MinDistance = FMath::Min( MinDistanceSq, FMath::Sqrt( MinDistanceSq ) );
 
 
 
                    //@ROCK - Dustin: Used for more accurate texture priority calculations
                    StreamingTexture.ScreenSizeInTexels = FMath::Max(StreamingTexture.ScreenSizeInTexels, ScreenSizeInTexels);
                }
                else
                {
                    WantedMipCount = FFloatMipLevel::FromMipLevel(StreamingTexture.MaxAllowedMips);
                    ...
                    //@ROCK - Dustin: Used for more accurate texture priority calculations
                    StreamingTexture.ScreenSizeInTexels = MAX_TEXEL_SIZE*MAX_TEXEL_SIZE;
                }
            }
        }
#else
...
void FStreamingManagerTexture::CalcDynamicWantedMips()
{
    ...
    for ( int32 Index=0; Index < StreamingTextures.Num() - PrefetchCount; ++Index )
    {
        ...
        StreamingTexture.ScreenSizeInTexels = 0.0f; //@ROCK - Dustin: Used for more accurate texture priority calculations
    }
    // The last ones
                ...
                if ( bIsPrimitiveUpToDate )
                {
                    ...
                    if ( bIsPrimitiveUpToDate )
                    {
                        ...
                        for( int32 ViewIndex=0; ViewIndex < ThreadNumViews(); ViewIndex++ )
                        {
                            ...
                            if (fabs(ViewInfo.ScreenSize) < SMALL_NUMBER)
                            {
                                //UE_LOG(LogContentStreaming, Log,TEXT("Avoid divided by zero"));
                            }
                            else
                            {
                                // Virtuos SHL: ignore bicubic upscaling percentage in screen size
...
#endif
#if VIRTUOS_PAIR_UE3UE4
                                    FMath::Lerp(ViewInfo.FOVScreenSize, ViewInfo.ScreenSize, FMath::Min(StreamingTexture.LastRenderTime/10.0f, 1.0f)) * ViewInfo.BoostFactor * ScreenSizeFactor; //@ROCK - Dustin: Changed to use FOV Screen size, for better results when zoomed in/out
#else
                                    ViewInfo.FOVScreenSize * ViewInfo.BoostFactor * ScreenSizeFactor; //@Virtuos [Zhangheng_a]: take fov into account and always use 1080p.
#endif
                            }
...
void FStreamMemoryTracker::GameThread_BeginUpgrade( const UTexture2D& Texture )
{
    TSize PreviousRequestedSize = Texture.CalcTextureMemorySize( Texture.PreviousRequestedMips );
    TSize RequestedSize = Texture.CalcTextureMemorySize( Texture.RequestedMips );
 
 
    if ( RequestedSize > PreviousRequestedSize )
    {
        // Plan on allocating new mips.
        FPlatformAtomics::InterlockedAdd( &PendingStreamIn, RequestedSize - PreviousRequestedSize );
    }
 
    check(RequestedSize > Texture.CalcTextureMemorySize( Texture.ResidentMips ));
}
 
void FStreamMemoryTracker::RenderThread_Upgrade( const UTexture2D& Texture)
{
    TSize PreviousRequestedSize = Texture.CalcTextureMemorySize( Texture.PreviousRequestedMips );
    TSize RequestedSize = Texture.CalcTextureMemorySize( Texture.RequestedMips );
    ...
    // Are we streaming in?
    check( RequestedSize > PreviousRequestedSize)
