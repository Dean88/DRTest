...
BEGIN_UNIFORM_BUFFER_STRUCT_WITH_CONSTRUCTOR(FViewUniformShaderParameters,ENGINE_API)
    ...
    DECLARE_UNIFORM_BUFFER_STRUCT_MEMBER(float,DepthOfFieldNearMaxBlurAmount)
    DECLARE_UNIFORM_BUFFER_STRUCT_MEMBER(float,DepthOfFieldFarMaxBlurAmount)
...
public:
    ...
    */
    ...
    // @VIRTUOS [RS] BEGIN: Special visibility rules for detective mode
    bool bIsXray;
    bool bIsThermal;
    bool bIsScanMode;
    bool bIsReflection;//@Virtuos[wangsongwei] it need more research about this parameter, we havn't do much work about this mode, maybe will cause some error, check bug[6116] describe ??
    // @VIRTUOS [RS] END
    ...
    // @Virtuos [Ding Yifei]: Indicate whether to use special depth bias in mirror case
    bool bEnableCapeDepthBiasHack;
